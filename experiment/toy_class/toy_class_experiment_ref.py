...
if __name__ == '__main__':
    ...
    suff = "_toy_ref_" + version + "a_" + str(q).replace(".0", "")

    base_cost = 0.1
    cost_factor = 0.9
    toy_class_exp_main(args, suff, lambda a: base_cost + cost_factor * a ** q,
                       lambda c: ((c - base_cost) / cost_factor) ** (1 / q), prior_var=0.5, var_factor=0.5,
                       fit=args.fit,
                       include_mi=True, version=version, interpolate_results=args.interpolate)
