...
def gpc_bald_ws_acquisition(X_Pool, Queries, **kwargs):
    ...
    c = (1 - 2 * scaled_alphas)**2 / (np.pi * np.log(2) * (1 - alpha_h))
    Average_Entropy = (1 - alpha_h) * np.exp(- c * m**2 / (1 + 2 * c * s)) / np.sqrt((1 + 2 * c * s)) + alpha_h
    Average_Entropy[:, scaled_alphas.reshape(-1) == 0.5, 0] = entropy(0.5)
    assert Average_Entropy.shape == (X_Pool.shape[0], alphas.shape[0], 1)
...
#                     gpc_bald_ws_acquisition, lambda a: (2 * a - 1) ** q, lambda c: 0.5 * (c ** (1 / q) + 1), args,
#                     betas=np.linspace(0.8, 1, 100), metric=acc_metric, metric_name='ACC',
#                     dir=args.save_dir + "GPC_BALD_WS_test", e=0, ref_model=False)
