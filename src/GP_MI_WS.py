...
# from src.experiment_utils import parse_args, round_floor, rbf_kernel, \
...
def mi_ws_acquisition(X_Pool, Queries, **kwargs):

    model = kwargs["model"]
    betas = kwargs["betas"]
    label_cost = kwargs["label_cost"]

    var_noisy_y_given_f = model.var_noisy_y_given_f(X_Pool.reshape((-1, 1, 1)), precision=betas.reshape((1, -1, 1)))
    #assert var_noisy_y_given_f.shape == (X_Pool.shape[0], betas.shape[0], 1)
    s = model.predict_model_var(X_Pool).reshape((X_Pool.shape[0], 1, -1))
    assert s.shape == (X_Pool.shape[0], 1, 1)

    var_y = s + var_noisy_y_given_f[:, -1].reshape(-1, 1, 1)
    var_noisy_y_given_y = model.var_noisy_y_given_y(var_y, s, precision=betas.reshape((1, -1, 1)))
    #assert var_noisy_y_given_y.shape == (X_Pool.shape[0], betas.shape[0], 1)

    Average_Entropy = np.sum(np.log(var_noisy_y_given_y), axis=-1)
    assert Average_Entropy.shape == (X_Pool.shape[0], betas.shape[0]) or Average_Entropy.shape == (1, betas.shape[0])

    Entropy_Average = np.sum(np.log(s + var_noisy_y_given_f),  axis=-1)
...
#                     mi_ws_acquisition, lambda a: a, lambda c: c, args,
#                     dir=args.save_dir + "MI_WS_test", e=0, ref_model=False)
